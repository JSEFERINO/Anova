...
# ============================================================
# PESTAÑA 5: FACTORIAL 2² (CON GRÁFICO DE COMBINACIONES)
# ============================================================
with tab5:
    col1, col2 = st.columns([1, 2])
    with col1:
    ...
    with col2:
        if 'f22' in st.session_state.resultados_anova:
            ...
            # --- Gráfico de Barras de Medias por Combinación ---
            medias = df.groupby(['A', 'B'])['Y'].mean().reset_index()
            fig3 = px.bar(medias, x='A', y='Y', color='B',
            ...
            with st.form(key="form_pruebas_f22"):
                metodo = st.radio("Selecciona el método:",
                                  ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"],
                                  horizontal=True, key="metodo_f22_form")
                factores_prueba = st.multiselect("Factores para probar:",
                                                 ['A', 'B'],
                                                 default=['A', 'B'],
                                                 key="factores_f22_form")
