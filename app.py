            ...
            if st.button("🔍 Calcular ANOVA", key="dca_btn"):
                ...
                if modelo is not None:
                    ...
                    if len(df['Tratamiento'].unique()) > 2:
                        ...
                        metodo = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_dca")
                        ...
                        if metodo == "Tukey HSD":
                            ...
                            if lsd_df is not None:
                                st.write(f"**MSE = {mse:.4f}, GL Error = {gl_error}**")
                                st.dataframe(lsd_df)
                                for linea in interpretar_lsd(lsd_df):
                                    st.write(linea)
            ...
            if st.button("🔍 Calcular", key="dbca_btn"):
                ...
                if modelo is not None:
                    ...
                    if len(df['Tratamiento'].unique()) > 2:
                        ...
                        metodo = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_dbca")
        ...
        if df_temp is not None and len(df_temp.columns) >= 4:
            ...
            if st.button("🔍 Calcular", key="cl_btn"):
                ...
                if modelo is not None:
                    ...
                    if len(df['Tratamiento'].unique()) > 2:
                        st.subheader("🔬 Pruebas Múltiples para Tratamientos")
                        metodo = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_cl")
        ...
        if df_temp is not None and len(df_temp.columns) >= 5:
            ...
            if st.button("🔍 Calcular", key="cgl_btn"):
                ...
                if modelo is not None:
                    ...
                    # Pruebas Múltiples para Tratamiento
                    if len(df['Tratamiento'].unique()) > 2:
                        st.subheader("🔬 Pruebas Múltiples para Tratamiento")
                        metodo = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_cgl_trat")
                    ...
                    # --- NUEVO: Pruebas Múltiples para Greco ---
                    if len(df['Greco'].unique()) > 2:
                        st.subheader("🔬 Pruebas Múltiples para Greco")
                        metodo_greco = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_cgl_greco")
            ...
            if st.button("🔍 Calcular", key="f22_btn"):
                ...
                if modelo is not None:
                    ...
                    if len(df['A'].unique()) > 2:
                        st.subheader("🔬 Pruebas Múltiples para Factor A")
                        metodo = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_f22_a")
                        ...
                        if metodo == "Tukey HSD":
                            ...
                            lsd_df, mse, gl_error = realizar_lsd(df, 'A', 'Y')
                            if lsd_df is not None:
                                st.write(f"**MSE = {mse:.4f}, GL Error = {gl_error}**")
                                st.dataframe(lsd_df)
                                for linea in interpretar_lsd(lsd_df):
                    ...
                    if len(df['B'].unique()) > 2:
                        st.subheader("🔬 Pruebas Múltiples para Factor B")
                        metodo_b = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_f22_b")

                        if metodo_b == "Tukey HSD":
                            ...
                            lsd_df, mse, gl_error = realizar_lsd(df, 'B', 'Y')
                            if lsd_df is not None:
                                st.write(f"**MSE = {mse:.4f}, GL Error = {gl_error}**")
                                st.dataframe(lsd_df)
                                for linea in interpretar_lsd(lsd_df):
...
# ============================================================
# PESTAÑA 6: FACTORIAL 2³ (ACTUALIZADA)
# ============================================================
with tab6:
    col1, col2 = st.columns([1, 2])
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 4:
            ...
            if st.button("🔍 Calcular", key="f23_btn"):
                ...
                if modelo is not None:
                    ...
                    # --- NUEVO: Selector de factores para Tukey HSD (Movido a expander para evitar recarga) ---
                    with st.expander("🔬 Pruebas Múltiples (Tukey HSD / LSD)"):
                        st.markdown("Selecciona los factores para los que deseas realizar la prueba:")

                        # Selector de factores
                        factores_disponibles = ['A', 'B', 'C']
                        factores_seleccionados = st.multiselect(
                            "Factores para prueba HSD/LSD:",
                            factores_disponibles,
                            default=['A'],
                            key="tukey_factores_f23_expander"
                        )

                        # Selector de método
                        metodo = st.radio("Método:", ["Tukey HSD", "LSD"], horizontal=True, key="metodo_f23")
                        ...
                        if not factores_seleccionados:
                            ...
                            for factor in factores_seleccionados:
                                ...
                                if len(df[factor].unique()) > 2:
                                    ...
                                    st.info(f"ℹ️ El factor {factor} tiene solo 2 niveles, no se requiere prueba.")
