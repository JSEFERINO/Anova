...
# Nuevo estado para pruebas múltiples
if 'pruebas_multiples' not in st.session_state:
    st.session_state.pruebas_multiples = {}
# NUEVO: Estado persistente para la selección en Factorial 2³
if 'f23_factores_seleccionados' not in st.session_state:
    st.session_state.f23_factores_seleccionados = ['A']
if 'f23_metodo' not in st.session_state:
    st.session_state.f23_metodo = "Tukey HSD"

# ============================================================
# NUEVA FUNCIÓN: SELECTOR DE TIPO DE ANOVA
# ============================================================

def selector_tipo_anova(key_suffix="", show_help=True):
    """
    Muestra un selector para elegir el tipo de ANOVA (I, II o III)
    """
    if show_help:
        st.markdown("""
        **📊 Tipos de ANOVA:**
        - **Tipo I (Secuencial):** El orden de entrada importa. Útil para modelos jerárquicos.
        - **Tipo II (Parcial):** Prueba cada efecto después de todos los demás del mismo nivel. Recomendado para diseños balanceados.
        - **Tipo III (Parcial con interacción):** Prueba cada factor controlando por todos los demás. Útil para diseños no balanceados.
        """)
    ...
    tipo_anova = st.radio(
        ...
        help="Selecciona el tipo de suma de cuadrados para el análisis"
    )
...
# ============================================================
# NUEVA FUNCIÓN: ANOVA MEJORADA CON MEAN SQUARE Y TIPO
# ============================================================

def analizar_anova_mejorada(df, formula, titulo="ANOVA", key_suffix="", tipo="dca", anova_type=2):
    """
    Realiza ANOVA con selección de tipo I, II o III
    y muestra Mean Square en la tabla
    """
    try:
        ...
        # Obtener tabla ANOVA según el tipo seleccionado
        anova_table = sm.stats.anova_lm(modelo, typ=anova_type)

        # Calcular Mean Square si no existe
        if 'mean_sq' not in anova_table.columns:
            anova_table['mean_sq'] = anova_table['sum_sq'] / anova_table['df']

        # Reordenar columnas para que Mean Square aparezca después de df
        columnas_orden = []
        for col in anova_table.columns:
            if col.lower() not in ['mean_sq', 'mean square', 'ms'] and col not in ['mean_sq']:
                columnas_orden.append(col)

        # Insertar Mean Square después de df
        df_index = None
        for i, col in enumerate(columnas_orden):
            if col.lower() == 'df':
                df_index = i
                break

        if df_index is not None:
            # Reordenar con Mean Square después de df
            columnas_final = []
            for i, col in enumerate(columnas_orden):
                columnas_final.append(col)
                if i == df_index:
                    columnas_final.append('mean_sq')
            anova_table = anova_table[columnas_final]
        else:
            # Si no encuentra df, poner mean_sq al final
            columnas_final = [col for col in anova_table.columns if col != 'mean_sq']
            columnas_final.append('mean_sq')
            anova_table = anova_table[columnas_final]
...
# ============================================================
# NUEVA FUNCIÓN: GRÁFICOS DE INTERACCIÓN MULTIFACTORIAL
# ============================================================

def graficos_interaccion_multifactorial(df, factores, respuesta, max_pares=6):
    """
    Genera gráficos de interacción para múltiples factores
    """
    if len(factores) < 2:
        return None

    # Calcular el número de pares de factores a mostrar
    num_pares = len(list(combinations(factores, 2)))
    mostrar_pares = min(num_pares, max_pares)

    # Seleccionar los pares con mayor interacción (opcional)
    pares_seleccionados = list(combinations(factores, 2))[:mostrar_pares]

    # Crear subplots
    n_filas = (mostrar_pares + 1) // 2
    fig = make_subplots(
        rows=n_filas,
        cols=2,
        subplot_titles=[f"Interacción {f1} × {f2}" for f1, f2 in pares_seleccionados]
    )

    for idx, (f1, f2) in enumerate(pares_seleccionados):
        row = idx // 2 + 1
        col = idx % 2 + 1

        try:
            # Calcular medias para el gráfico de interacción
            medias = df.groupby([f1, f2])[respuesta].mean().reset_index()

            # Crear líneas para cada nivel del segundo factor
            for nivel in medias[f2].unique():
                datos_nivel = medias[medias[f2] == nivel]
                fig.add_trace(
                    go.Scatter(
                        x=datos_nivel[f1],
                        y=datos_nivel[respuesta],
                        mode='lines+markers',
                        name=f"{f2}={nivel}",
                        legendgroup=f"{f2}={nivel}",
    ...
    fig.update_layout(
        height=min(400, 200 * n_filas),
        showlegend=True,
        title_text="Gráficos de Interacción entre Factores"
    )
...
def graficos_interaccion_heatmap(df, factores, respuesta):
    """
    Genera mapa de calor de interacciones entre factores
    """
    if len(factores) < 2:
        return None

    # Crear matriz de medias para el heatmap
    try:
        # Usar los dos primeros factores para el heatmap
...
# ============================================================
# NUEVAS FUNCIONES DE GRÁFICOS ADICIONALES
# ============================================================

def graficar_boxplot_por_factor(df, factores, respuesta):
    """Genera boxplots individuales por cada factor (como en la imagen 1)"""
    if not factores:
        return None
...
def graficar_sumas_cuadrados(anova_table):
    """Genera el gráfico de barras de Sumas de Cuadrados (imagen 3)"""
    try:
        # Extraer datos, ignorando la fila de 'Residual' o 'Error' si se desea, 
        # pero aquí se muestra todo como en la imagen.
        df_sc = anova_table.reset_index()
        df_sc = df_sc.rename(columns={'index': 'Fuente'})
        
        # Identificar la columna de Suma de Cuadrados
        col_sc = 'sum_sq' if 'sum_sq' in df_sc.columns else 'Sum Sq' if 'Sum Sq' in df_sc.columns else None
        if not col_sc:
            return None
            
        # Filtrar la fila de Total si existe
        if 'Total' in df_sc['Fuente'].values:
            df_sc = df_sc[df_sc['Fuente'] != 'Total']
            
        fig = px.bar(df_sc, x='Fuente', y=col_sc, 
                     title="Gráfico de Sumas de Cuadrados (SC)",
                     color='Fuente', text_auto='.2f')
        
        # Aplicar escala logarítmica si hay mucha diferencia (como en la imagen)
        if df_sc[col_sc].max() / df_sc[col_sc].min() > 100:
            fig.update_yaxes(type="log", title_text="Suma de Cuadrados (SC) - Escala Log")
        else:
            fig.update_yaxes(title_text="Suma de Cuadrados (SC)")
...
def graficar_verificacion_modelo_completa(modelo, df, factor, respuesta):
    """Genera la verificación completa del modelo (imagen 5)"""
    if modelo is None:
        return None
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=[
            "1. Normalidad de los residuos (Q-Q)",
    ...
    fig.add_trace(go.Scatter(x=osm, y=osr, mode='markers', marker=dict(color='blue', size=6), name='Residuos'), row=1, col=1)
    fig.add_trace(go.Scatter(x=[min(osm), max(osm)], y=[min(osm)*slope + intercept, max(osm)*slope + intercept], 
                             mode='lines', line=dict(color='red', dash='dash'), name='Ajuste Normal'), row=1, col=1)

    # 2. Residuos vs Ajustados
    fig.add_trace(go.Scatter(x=predichos, y=residuos, mode='markers', marker=dict(color='green', size=6), name='Residuos'), row=1, col=2)
    fig.add_hline(y=0, line_dash="dash", line_color="red", row=1, col=2)

    # 3. Residuos vs Orden de Corrida
    fig.add_trace(go.Scatter(x=run_order, y=residuos, mode='lines+markers', marker=dict(color='orange', size=6), name='Residuos'), row=2, col=1)
    fig.add_hline(y=0, line_dash="dash", line_color="red", row=2, col=1)

    # 4. Histograma
    fig.add_trace(go.Histogram(x=residuos, marker_color='purple', name='Frecuencia', nbinsx=15), row=2, col=2)
    
    # Ajustar ejes
    fig.update_xaxes(title_text="Cuantiles Teóricos", row=1, col=1)
    fig.update_yaxes(title_text="Residuos Estandarizados", row=1, col=1)
    fig.update_xaxes(title_text="Valores Ajustados", row=1, col=2)
    fig.update_yaxes(title_text="Residuos Estandarizados", row=1, col=2)
    fig.update_xaxes(title_text="Orden de Corrida", row=2, col=1)
    fig.update_yaxes(title_text="Residuos Estandarizados", row=2, col=1)
    fig.update_xaxes(title_text="Residuos", row=2, col=2)
    fig.update_yaxes(title_text="Frecuencia", row=2, col=2)

    fig.update_layout(height=700, showlegend=False, title_text="Verificación de Supuestos del Modelo")
    return fig

def graficar_interaccion_mejorada(df, f1, f2, respuesta):
    """Genera gráfico de interacción con líneas y puntos, con colores y estilos (imagen 4)"""
    medias = df.groupby([f1, f2])[respuesta].mean().reset_index()
    
    fig = px.line(medias, x=f1, y=respuesta, color=f2, markers=True,
                  title=f"Interacción {f1} × {f2}",
                  labels={f1: f1, respuesta: respuesta, f2: f2})
    
    # Añadir texto con los valores de las medias
    for i, row in medias.iterrows():
        fig.add_annotation(
            x=row[f1], y=row[respuesta],
...
# ============================================================
# NUEVA FUNCIÓN: PRUEBA LSD (DIFERENCIA MÍNIMA SIGNIFICATIVA)
# ============================================================

def realizar_lsd(df, factor, respuesta, alpha=0.05):
    """Realiza prueba de Diferencia Mínima Significativa (LSD)"""
    try:
        # Calcular medias y tamaño de muestra por grupo
        medias = df.groupby(factor)[respuesta].mean()
        n_grupos = len(medias)
        n_total = len(df)
        
        # Calcular ANOVA para obtener MSE
        formula = f'{respuesta} ~ C({factor})'
        modelo = ols(formula, data=df).fit()
        anova_table = sm.stats.anova_lm(modelo, typ=2)
        
        # Obtener MSE y grados de libertad del error
        mse = anova_table['sum_sq'].iloc[-1] / anova_table['df'].iloc[-1]
        gl_error = anova_table['df'].iloc[-1]
        
        # Calcular t crítico
        t_crit = stats.t.ppf(1 - alpha/2, gl_error)
        
        # Calcular LSD para cada par
        resultados = []
        ...
        # Crear DataFrame similar a Tukey
        lsd_df = pd.DataFrame(resultados)
        return lsd_df, mse, gl_error
        
    except Exception as e:
        return None, None, None

def interpretar_lsd(lsd_df):
    """Interpreta los resultados de LSD"""
    interpretacion = []
    ...
    try:
        ...
        if len(significativas) == 0:
            interpretacion.append("📊 **LSD:** No hay diferencias significativas entre ningún par de tratamientos")
            return interpretacion
        
        interpretacion.append(f"📊 **LSD:** Se encontraron {len(significativas)} diferencias significativas")
        ...
        interpretacion.append(f"❌ Error al interpretar LSD: {str(e)}")
...
def mostrar_graficos_residuos_directo(modelo, df, factor=None, respuesta=None, key_suffix="", titulo="Gráficos de Residuos"):
    """
    Muestra los gráficos de residuos directamente sin botón (Ahora con verificación completa)
    """
    if modelo is None:
    ...
    with st.spinner("Generando gráficos de verificación del modelo..."):
        # Usar la nueva función de verificación completa
        fig_residuos = graficar_verificacion_modelo_completa(modelo, df, factor, respuesta)
...
# ============================================================
# FUNCIONES DE ANÁLISIS Y CONCLUSIONES - CORREGIDAS
# ============================================================

def generar_conclusiones_anova(anova_table, modelo, df, factor, respuesta, alpha=0.05):
    conclusiones = []

    # Buscar el p-valor en la tabla ANOVA
    p_valor = None
    for col in anova_table.columns:
        if 'PR(>F)' in col or 'P-Value' in col or 'p-value' in col.lower():
            p_valor = anova_table[col].iloc[0]
            break
        elif 'F' in col and 'F-ratio' in col.lower():
            # Si no encuentra p-valor, intenta usar F
            try:
                f_val = anova_table[col].iloc[0]
                # Calcular p-valor aproximado
                df1 = anova_table['DF'].iloc[0] if 'DF' in anova_table.columns else anova_table['df'].iloc[0]
                df2 = anova_table['DF'].iloc[-1] if 'DF' in anova_table.columns else anova_table['df'].iloc[-1]
                from scipy.stats import f
                p_valor = f.sf(f_val, df1, df2)
            except:
                p_valor = None
            break
    ...
    if p_valor is None:
        ...
        for col in anova_table.columns:
            if 'P-Value' in col or 'PR(>F)' in col:
                p_valor = anova_table[col].iloc[0]
                break

    if p_valor is None:
        # Si no se puede obtener, usar el primer valor numérico
        for col in anova_table.columns:
            if col not in ['Sum Sq', 'DF', 'Mean Sq']:
                try:
                    p_valor = anova_table[col].iloc[0]
                    break
                except:
    ...
    if p_valor is None:
        conclusiones.append("⚠️ No se pudo obtener el p-valor de la tabla ANOVA")
        return conclusiones, None
...
def generar_conclusiones_factorial(anova_table, modelo, df, factores, respuesta, alpha=0.05):
    ...
    # Buscar p-valores en la tabla ANOVA
...
# ============================================================
# FUNCIÓN PARA ANÁLISIS INTERACTIVO (ACTUALIZADA)
# ============================================================
...
def analizar_diseno_interactivo(df, factores_def, respuestas_def):
    ...
    # Selector de tipo de ANOVA para el análisis interactivo
    tipo_anova_interactivo = selector_tipo_anova("interactivo", show_help=False)
    ...
    if len(factores) == 1:
        ...
        try:
            modelo = ols(formula, data=df).fit()
            anova_table = sm.stats.anova_lm(modelo, typ=tipo_anova_interactivo)

            # Calcular Mean Square
            if 'mean_sq' not in anova_table.columns:
                anova_table['mean_sq'] = anova_table['sum_sq'] / anova_table['df']
            ...
            fig_sc = graficar_sumas_cuadrados(anova_table)
            if fig_sc:
        ...
        # Selector de nivel de interacciones
        ...
        # Generar fórmula según nivel de interacciones
        if nivel_interaccion == "main":
        ...
        # Convertir factores a categóricos
        for f in factores:
        ...
        # ============================================================
        # NUEVA SECCIÓN: GRÁFICOS DE INTERACCIÓN PARA MÚLTIPLES FACTORES
        # ============================================================
        st.subheader("📊 Gráficos de Interacción")

        # Mostrar gráficos de interacción para pares de factores
        if len(factores) >= 2:
            # Selector de cuántos pares mostrar
            num_pares = len(list(combinations(factores, 2)))
            max_pares_mostrar = st.slider(
                "Número de pares de interacción a mostrar:",
                min_value=1,
                max_value=min(num_pares, 8),
                value=min(4, num_pares),
                key="num_pares_interaccion"
            )

            # Generar gráficos de interacción
            fig_interaccion = graficos_interaccion_multifactorial(
                df, factores, respuesta_seleccionada, max_pares_mostrar
            )
            ...
            fig_heatmap = graficos_interaccion_heatmap(df, factores, respuesta_seleccionada)
            if fig_heatmap is not None:
                st.plotly_chart(fig_heatmap, use_container_width=True)

        # Visualización de medias por factor
        st.subheader("📊 Medias por Factor")
...
# ============================================================
# PESTAÑA 1: ANOVA UN FACTOR (ACTUALIZADA)
# ============================================================
with tab1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 2:
            ...
            if st.button("🔍 Calcular ANOVA", key="dca_btn"):
                ...
                if modelo is not None:
                    ...
                    # Guardar en session_state para pruebas múltiples
    ...
    with col2:
        if 'dca' in st.session_state.resultados_anova:
            ...
            # --- PRUEBAS MÚLTIPLES CON FORMULARIO (Evita recarga) ---
            ...
            with st.form(key="form_pruebas_dca"):
                ...
                if submitted:
                    if metodo == "Tukey HSD":
                        ...
                        lsd_df, mse, gl_error = realizar_lsd(df, 'Tratamiento', 'Valor')
                        if lsd_df is not None:
                            st.session_state.pruebas_multiples['dca_resultado'] = ('lsd', lsd_df, mse, gl_error)

            # Mostrar resultados de la prueba si existen
            if 'dca_resultado' in st.session_state.pruebas_multiples:
                ...
                if res[0] == 'tukey':
                    ...
                    st.write(f"**MSE = {res[2]:.4f}, GL Error = {res[3]}**")
                    st.dataframe(res[1])
...
# ============================================================
# PESTAÑA 2: DISEÑO EN BLOQUES (ACTUALIZADA)
# ============================================================
with tab2:
    col1, col2 = st.columns([1, 2])
    with col1:
    ...
    with col2:
        if 'dbca' in st.session_state.resultados_anova:
            ...
            # --- EXISTENTE: Medias por Tratamiento ---
            medias = df.groupby('Tratamiento')['Valor'].mean().reset_index()
            fig2 = px.bar(medias, x='Tratamiento', y='Valor', title="Medias por Tratamiento", color='Tratamiento', text_auto=True)
            fig2.update_layout(showlegend=False, height=300)
            fig2.update_traces(texttemplate='%{text:.3f}', textposition='outside')
            ...
            with st.form(key="form_pruebas_dbca"):
                ...
                if submitted:
                    if metodo == "Tukey HSD":
                        ...
                        lsd_df, mse, gl_error = realizar_lsd(df, 'Tratamiento', 'Valor')
                        if lsd_df is not None:
                            st.session_state.pruebas_multiples['dbca_resultado'] = ('lsd', lsd_df, mse, gl_error)
...
# ============================================================
# PESTAÑA 3: CUADRADO LATINO (ACTUALIZADA)
# ============================================================
with tab3:
    col1, col2 = st.columns([1, 2])
    with col1:
            ...
            with st.form(key="form_pruebas_cl"):
                ...
                if submitted:
                    if metodo == "Tukey HSD":
                        ...
                        lsd_df, mse, gl_error = realizar_lsd(df, 'Tratamiento', 'Valor')
                        if lsd_df is not None:
                            st.session_state.pruebas_multiples['cl_resultado'] = ('lsd', lsd_df, mse, gl_error)
...
# ============================================================
# PESTAÑA 4: CUADRADO GRECO-LATINO (ACTUALIZADA)
# ============================================================
with tab4:
    col1, col2 = st.columns([1, 2])
    with col1:
    ...
    with col2:
        if 'cgl' in st.session_state.resultados_anova:
            ...
            # --- PRUEBAS MÚLTIPLES CON FORMULARIO (Tratamiento y Greco) ---
            st.subheader("🔬 Pruebas Múltiples")
            
            # Para Tratamiento
            st.markdown("#### 📌 Para Tratamiento")
            with st.form(key="form_pruebas_cgl_trat"):
                ...
                if submitted_trat:
                    if metodo_trat == "Tukey HSD":
                        ...
                        lsd_df, mse, gl_error = realizar_lsd(df, 'Tratamiento', 'Valor')
                        if lsd_df is not None:
                            st.session_state.pruebas_multiples['cgl_trat_resultado'] = ('lsd', lsd_df, mse, gl_error)
            ...
            with st.form(key="form_pruebas_cgl_greco"):
                ...
                if submitted_greco:
                    if metodo_greco == "Tukey HSD":
                        ...
                        lsd_df, mse, gl_error = realizar_lsd(df, 'Greco', 'Valor')
                        if lsd_df is not None:
                            st.session_state.pruebas_multiples['cgl_greco_resultado'] = ('lsd', lsd_df, mse, gl_error)
            ...
            if st.button("🗑️ Limpiar resultados", key="limpiar_cgl"):
                ...
                if 'cgl_trat_resultado' in st.session_state.pruebas_multiples:
                    del st.session_state.pruebas_multiples['cgl_trat_resultado']
                if 'cgl_greco_resultado' in st.session_state.pruebas_multiples:
                    del st.session_state.pruebas_multiples['cgl_greco_resultado']
                st.rerun()

# ============================================================
# PESTAÑA 5: FACTORIAL 2² (ACTUALIZADA)
# ============================================================
with tab5:
    col1, col2 = st.columns([1, 2])
    with col1:
    ...
    with col2:
        if 'f22' in st.session_state.resultados_anova:
            ...
            # --- NUEVO: Gráfico de Boxplot por Factor ---
            st.write("**Boxplot por Factor:**")
            fig_box = graficar_boxplot_por_factor(df, ['A', 'B'], 'Y')
            if fig_box:
                st.plotly_chart(fig_box, use_container_width=True)

            # --- NUEVO: Gráfico de Interacción Mejorado ---
            st.write("**Gráfico de Interacción Mejorado:**")
            fig_inter = graficar_interaccion_mejorada(df, 'A', 'B', 'Y')
            if fig_inter:
                st.plotly_chart(fig_inter, use_container_width=True)

            # --- EXISTENTE: Boxplot por combinación ---
            ...
            with st.form(key="form_pruebas_f22"):
                ...
                factores_prueba = st.multiselect("Factores para probar:", ['A', 'B'], default=['A'], key="factores_f22_form")
                    ...
                    for factor in factores_prueba:
                        if len(df[factor].unique()) > 2:
                            if metodo == "Tukey HSD":
                                ...
                                lsd_df, mse, gl_error = realizar_lsd(df, factor, 'Y')
                                if lsd_df is not None:
                                    resultados_multiples[factor] = ('lsd', lsd_df, mse, gl_error)
            ...
            if 'f22_resultado' in st.session_state.pruebas_multiples:
                res_dict = st.session_state.pruebas_multiples['f22_resultado']
...
# ============================================================
# PESTAÑA 6: FACTORIAL 2³ (ACTUALIZADA - CORREGIDA)
# ============================================================
with tab6:
    col1, col2 = st.columns([1, 2])
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 4:
            ...
            if st.button("🔍 Calcular", key="f23_btn"):
                ...
                if modelo is not None:
                    ...
                    # ============================================================
                    # GRÁFICOS DE INTERACCIÓN PARA FACTORIAL 2³
                    # ============================================================
                    st.subheader("📊 Gráficos de Interacción")
    ...
    with col2:
        if 'f23' in st.session_state.resultados_anova:
            ...
            # --- NUEVO: Gráfico de Interacción Mejorado (A x B) ---
            ...
            # --- PRUEBAS MÚLTIPLES CON FORMULARIO (CORREGIDO) ---
            st.subheader("🔬 Pruebas Múltiples")
            
            # Usamos session_state para mantener la selección persistente
            # 1. Selector de método (fuera del formulario para persistencia)
            metodo = st.radio(
                "Selecciona el método:", 
                ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], 
                horizontal=True, 
                key="metodo_f23_persistente",
                index=0 if st.session_state.f23_metodo == "Tukey HSD" else 1
            )
            st.session_state.f23_metodo = metodo

            # 2. Selector de factores (fuera del formulario)
            factores_prueba = st.multiselect(
                "Factores para probar (selecciona uno o varios):", 
                ['A', 'B', 'C'], 
                default=st.session_state.f23_factores_seleccionados,
                key="factores_f23_persistente"
            )
            st.session_state.f23_factores_seleccionados = factores_prueba

            # 3. Botón de calcular dentro de un formulario para evitar recargas
            with st.form(key="form_pruebas_f23_corregido"):
                submitted = st.form_submit_button("Calcular Pruebas")
                    ...
                    for factor in factores_prueba:
                        if len(df[factor].unique()) > 2:
                            if metodo == "Tukey HSD":
                                ...
                                if lsd_df is not None:
                                    resultados_multiples[factor] = ('lsd', lsd_df, mse, gl_error)
                    st.session_state.pruebas_multiples['f23_resultado'] = resultados_multiples

            # Mostrar resultados
            if 'f23_resultado' in st.session_state.pruebas_multiples:
                res_dict = st.session_state.pruebas_multiples['f23_resultado']
                for factor, res in res_dict.items():
                    st.markdown(f"#### 📌 Resultados para Factor {factor}")
                    if res[0] == 'tukey':
                        tukey_df = pd.DataFrame(data=res[1].summary().data[1:], columns=res[1].summary().data[0])
            ...
            if st.button("🗑️ Limpiar resultados", key="limpiar_f23"):
                ...
                if 'f23_resultado' in st.session_state.pruebas_multiples:
                    del st.session_state.pruebas_multiples['f23_resultado']
                st.rerun()

# ============================================================
# PESTAÑA 7: DISEÑO 2^k (ACTUALIZADA CON GRÁFICOS DE INTERACCIÓN)
# ============================================================
with tab7:
    col1, col2 = st.columns([1, 2])
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= k+1:
            ...
            if st.button("🔍 Calcular", key="2k_btn"):
                ...
                modelo, anova = analizar_anova_mejorada(df, formula, f"Factorial 2^{k}", "d2k", "d2k", tipo_anova)
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    ...
                    # ============================================================
                    # GRÁFICOS DE INTERACCIÓN PARA 2^k
                    # ============================================================
                    if k >= 2:
                        st.subheader("📊 Gráficos de Interacción")
    ...
    with col2:
        if 'd2k' in st.session_state.resultados_anova:
            ...
            # --- PRUEBAS MÚLTIPLES CON FORMULARIO ---
            st.subheader("🔬 Pruebas Múltiples")
            with st.form(key="form_pruebas_d2k"):
                metodo = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_d2k_form")
                factores_prueba = st.multiselect("Factores para probar:", factores, default=[factores[0]] if factores else [], key="factores_d2k_form")
                submitted = st.form_submit_button("Calcular Pruebas")
                    ...
                    for factor in factores_prueba:
                        if len(df[factor].unique()) > 2:
                            if metodo == "Tukey HSD":
                                ...
                                if lsd_df is not None:
                                    resultados_multiples[factor] = ('lsd', lsd_df, mse, gl_error)
                    st.session_state.pruebas_multiples['d2k_resultado'] = resultados_multiples

            if 'd2k_resultado' in st.session_state.pruebas_multiples:
                res_dict = st.session_state.pruebas_multiples['d2k_resultado']
                for factor, res in res_dict.items():
                    st.markdown(f"#### 📌 Resultados para {factor}")
                    if res[0] == 'tukey':
                        tukey_df = pd.DataFrame(data=res[1].summary().data[1:], columns=res[1].summary().data[0])
                        st.dataframe(tukey_df)
                        for linea in interpretar_tukey(res[1]):
            ...
            if st.button("🗑️ Limpiar resultados", key="limpiar_d2k"):
                ...
                if 'd2k_resultado' in st.session_state.pruebas_multiples:
                    del st.session_state.pruebas_multiples['d2k_resultado']
                st.rerun()

# ============================================================
# PESTAÑA 8: FRACCIONADO (ACTUALIZADA)
# ============================================================
with tab8:
    col1, col2 = st.columns([1, 2])
    with col1:
        ...
        # Selector de tipo de ANOVA
        tipo_anova = selector_tipo_anova("fracc")
        ...
        if df_temp is not None and len(df_temp.columns) >= k+1:
            ...
            if st.button("🔍 Calcular", key="fracc_btn"):
                ...
                modelo, anova = analizar_anova_mejorada(df, formula, f"Fraccionado 2^({k}-{p})", "fracc", "fracc", tipo_anova)
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state.pruebas_multiples['fracc'] = {'df': df, 'modelo': modelo, 'anova': anova}
                    ...
                    # ============================================================
                    # GRÁFICOS DE INTERACCIÓN PARA FRACCIONADO
                    # ============================================================
                    if k >= 2:
                        st.subheader("📊 Gráficos de Interacción")
                        ...
                        max_pares_mostrar = st.slider(
                            ...
                            max_value=min(num_pares, 8),
                            value=min(4, num_pares),
                            key="num_pares_interaccion_fracc"
                        )
    ...
    with col2:
        if 'fracc' in st.session_state.resultados_anova:
            ...
            # --- PRUEBAS MÚLTIPLES CON FORMULARIO ---
            st.subheader("🔬 Pruebas Múltiples")
            with st.form(key="form_pruebas_fracc"):
                metodo = st.radio("Selecciona el método:", ["Tukey HSD", "LSD (Diferencia Mínima Significativa)"], horizontal=True, key="metodo_fracc_form")
                factores_prueba = st.multiselect("Factores para probar:", factores, default=[factores[0]] if factores else [], key="factores_fracc_form")
                submitted = st.form_submit_button("Calcular Pruebas")
                    ...
                    for factor in factores_prueba:
                        if len(df[factor].unique()) > 2:
                            if metodo == "Tukey HSD":
                                ...
                                if lsd_df is not None:
                                    resultados_multiples[factor] = ('lsd', lsd_df, mse, gl_error)
                    st.session_state.pruebas_multiples['fracc_resultado'] = resultados_multiples

            if 'fracc_resultado' in st.session_state.pruebas_multiples:
                res_dict = st.session_state.pruebas_multiples['fracc_resultado']
                for factor, res in res_dict.items():
                    st.markdown(f"#### 📌 Resultados para {factor}")
                    if res[0] == 'tukey':
                        tukey_df = pd.DataFrame(data=res[1].summary().data[1:], columns=res[1].summary().data[0])
            ...
            if st.button("🗑️ Limpiar resultados", key="limpiar_fracc"):
                ...
                if 'fracc_resultado' in st.session_state.pruebas_multiples:
                    del st.session_state.pruebas_multiples['fracc_resultado']
                st.rerun()

# ============================================================
# PESTAÑA 9: MSR (ACTUALIZADA)
# ============================================================
with tab9:
    col1, col2 = st.columns([1, 2])
    with col1:
...
# ============================================================
# PESTAÑA 10: TAGUCHI (ACTUALIZADA)
# ============================================================
with tab10:
    col1, col2 = st.columns([1, 2])
    with col1:
        ...
        # Selector de tipo de ANOVA
        tipo_anova = selector_tipo_anova("taguchi")
        ...
        if df_temp is not None and len(df_temp.columns) >= n_fact+1:
            ...
            if st.button("🔍 Calcular Taguchi", key="taguchi_btn"):
                ...
                modelo, anova = analizar_anova_mejorada(df, formula, "Taguchi", "taguchi", "taguchi", tipo_anova)
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
...
# ============================================================
# PESTAÑA 12: ADITIVOS (ACTUALIZADA)
# ============================================================
with tab12:
    col1, col2 = st.columns([1, 2])
    with col1:
        ...
        # Selector de tipo de ANOVA
        tipo_anova = selector_tipo_anova("aditivos")
        ...
        if df_temp is not None and len(df_temp.columns) >= 4:
            ...
            if st.button("🔍 Calcular", key="aditivos_btn"):
                try:
                    ...
                    formula = 'Y ~ C1 + C2 + C3'
                    modelo = ols(formula, data=df).fit()
                    anova_table = sm.stats.anova_lm(modelo, typ=tipo_anova)
                    ...
                    st.write(f"**R² = {modelo.rsquared:.6f}**")
                    st.session_state['aditivos_resultado'] = {'df': df, 'modelo': modelo, 'anova': anova_table}
...
# ============================================================
# PESTAÑA 13: SPLIT-PLOT (ACTUALIZADA)
# ============================================================
with tab13:
    col1, col2 = st.columns([1, 2])
    with col1:
        ...
        # Selector de tipo de ANOVA
        tipo_anova = selector_tipo_anova("split")
            ...
            if st.button("🔍 Calcular", key="split_btn"):
                try:
                    ...
                    for col in ['F1', 'F2', 'F3']:
                        df[col] = df[col].astype('category')
                    formula = 'Y ~ F1 * F2 * F3'
                    modelo, anova = analizar_anova_mejorada(df, formula, "Split-Plot", "split", "split", tipo_anova)
                    if modelo is not None:
                        st.success("✅ ANOVA completado exitosamente")
                        st.session_state.pruebas_multiples['split'] = {'df': df, 'modelo': modelo, 'anova': anova}
                        ...
                        # ============================================================
                        # GRÁFICOS DE INTERACCIÓN PARA SPLIT-PLOT
                        # ============================================================
                        st.subheader("📊 Gráficos de Interacción")

                        factores_list = ['F1', 'F2', 'F3']
                        num_pares = len(list(combinations(factores_list, 2)))
                        max_pares_mostrar = st.slider(
                            "Número de pares de interacción a mostrar:",
                            min_value=1,
                            max_value=min(num_pares, 8),
                            value=min(3, num_pares),
                            key="num_pares_interaccion_split"
                        )
