...
def detectar_separador(texto):
    ...
    # Seleccionar el separador más común
...
def convertir_tipos(df):
    ...
    for col in df.columns:
        ...
        try:
            ...
            # Si no se puede, mantener como string/category
    ...
    try:
        ...
        try:
            ...
            for sep in [',', ';', '\t', '|']:
                ...
                # Si todo falla, usar el separador detectado
...
def procesar_datos_texto(texto, n_columnas=None):
    ...
    try:
        ...
        # Intentar determinar si es encabezado
        try:
            for v in valores:
                float(v.strip())
            tiene_encabezado = False
...
def obtener_datos_generales(key_suffix, n_columnas_esperadas=None):
    ...
    # Opciones de carga con clave única
    tipo_carga = st.radio(
    ...
    # Si hay datos guardados en session_state, mostrarlos
...
def analizar_anova(df, formula, titulo="ANOVA", key_suffix=""):
    """Realiza análisis ANOVA y muestra resultados"""
    try:
        # Asegurar que las variables categóricas estén como category
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 2:
            ...
            if st.button("🔍 Calcular ANOVA", key="dca_btn"):
                ...
                # Convertir Tratamiento a categórico
                df['Tratamiento'] = df['Tratamiento'].astype('category')
                
                modelo, anova = analizar_anova(df, 'Valor ~ C(Tratamiento)', "ANOVA Un Factor", "dca")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    # Mostrar gráficos en la columna derecha
                    st.session_state['mostrar_dca'] = True
    ...
    with col2:
        ...
        if 'dca' in st.session_state.resultados_anova:
            ...
            fig = px.box(df, x='Tratamiento', y='Valor', color='Tratamiento', 
                        title=f"Distribución por Tratamiento")
            ...
            if len(df['Tratamiento'].unique()) > 2:
                ...
                tukey = pairwise_tukeyhsd(df['Valor'], df['Tratamiento'])
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 3:
            ...
            if st.button("🔍 Calcular", key="dbca_btn"):
                ...
                modelo, anova = analizar_anova(df, 'Valor ~ C(Tratamiento) + C(Bloque)', "ANOVA en Bloques", "dbca")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state['mostrar_dbca'] = True
        elif df_temp is not None:
...
with tab3:
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 4:
            ...
            col_fila = st.selectbox(
                "Selecciona la columna de fila:",
                df_temp.columns.tolist(),
                key="fila_cl"
            )
            
            col_columna = st.selectbox(
                "Selecciona la columna de columna:",
                [col for col in df_temp.columns if col != col_fila],
                key="columna_cl"
            )
            
            col_tratamiento = st.selectbox(
                "Selecciona la columna de tratamiento:",
                [col for col in df_temp.columns if col not in [col_fila, col_columna]],
                key="tratamiento_cl"
            )
            
            col_respuesta = st.selectbox(
                "Selecciona la columna de respuesta (Valor):",
                [col for col in df_temp.columns if col not in [col_fila, col_columna, col_tratamiento]],
                key="respuesta_cl"
            )
            ...
            if st.button("🔍 Calcular", key="cl_btn"):
                ...
                modelo, anova = analizar_anova(df, 'Valor ~ C(Fila) + C(Columna) + C(Tratamiento)', "Cuadrado Latino", "cl")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state['mostrar_cl'] = True
        elif df_temp is not None:
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 5:
            ...
            if st.button("🔍 Calcular", key="cgl_btn"):
                ...
                modelo, anova = analizar_anova(df, 'Valor ~ C(Fila) + C(Columna) + C(Tratamiento) + C(Greco)', "Greco-Latino", "cgl")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state['mostrar_cgl'] = True
        elif df_temp is not None:
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 3:
            ...
            if st.button("🔍 Calcular", key="f22_btn"):
                ...
                modelo, anova = analizar_anova(df, 'Y ~ A * B', "Factorial 2²", "f22")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state['mostrar_f22'] = True
        elif df_temp is not None:
            st.warning("⚠️ Se necesitan al menos 3 columnas para el análisis")
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= 4:
            ...
            if st.button("🔍 Calcular", key="f23_btn"):
                ...
                modelo, anova = analizar_anova(df, 'Y ~ A * B * C', "Factorial 2³", "f23")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state['mostrar_f23'] = True
        elif df_temp is not None:
            st.warning("⚠️ Se necesitan al menos 4 columnas para el análisis")
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= k+1:
            ...
            if st.button("🔍 Calcular", key="2k_btn"):
                ...
                modelo, anova = analizar_anova(df, formula, f"Factorial 2^{k}", "d2k")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state['mostrar_d2k'] = True
        elif df_temp is not None:
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= k+1:
            ...
            if st.button("🔍 Calcular", key="fracc_btn"):
                ...
                modelo, anova = analizar_anova(df, formula, f"Fraccionado 2^({k}-{p})", "fracc")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state['mostrar_fracc'] = True
        elif df_temp is not None:
            st.warning(f"⚠️ Se necesitan {k+1} columnas para el análisis")
        ...
        if df_temp is not None and len(df_temp.columns) >= n_fact+1:
            ...
            if st.button("🔍 Calcular MSR", key="msr_btn"):
                try:
                    ...
                    modelo, anova = analizar_anova(df, formula, "MSR", "msr")
                    if modelo is not None:
                        st.success("✅ ANOVA completado exitosamente")
                        st.session_state['mostrar_msr'] = True
                except Exception as e:
    ...
    with col1:
        ...
        if df_temp is not None and len(df_temp.columns) >= n_fact+1:
            ...
            if st.button("🔍 Calcular Taguchi", key="taguchi_btn"):
                ...
                mapeo = {factores[i]: f'F{i+1}' for i in range(n_fact)}
                mapeo[col_respuesta] = 'Y'
                ...
                modelo, anova = analizar_anova(df, formula, "Taguchi", "taguchi")
                if modelo is not None:
                    st.success("✅ ANOVA completado exitosamente")
                    st.session_state['mostrar_taguchi'] = True
        elif df_temp is not None:
            st.warning(f"⚠️ Se necesitan {n_fact+1} columnas para el análisis")
        ...
        if df_temp is not None and len(df_temp.columns) >= 4:
            ...
            if st.button("🔍 Calcular", key="split_btn"):
                try:
                    ...
                    modelo, anova = analizar_anova(df, 'Y ~ F1 * F2 * F3', "Split-Plot", "split")
                    if modelo is not None:
                        st.success("✅ ANOVA completado exitosamente")
                        st.session_state['mostrar_split'] = True
                except Exception as e:
                    st.error(f"❌ Error: {str(e)}")
        elif df_temp is not None:
            st.warning("⚠️ Se necesitan 4 columnas para el análisis")
